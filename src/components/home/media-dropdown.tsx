...
const MediaDropdown = () => {
    ...
    const handleSendImage = async () => {
        ...
        try {
            ...
            toast.error("Failed to send requested image")
        } finally {
            setIsLoading(false)
        }
    }
...
const MediaImageDialog = ({isOpen, onClose, selectedImage, isLoading, handleSendImage}: MediaImageDialogProps) => {
    const [renderedImage, setRenederedImage] = useState<string | null>(null)

    useEffect(() => {
        if(!selectedImage) return;
        const reader = new FileReader()
        reader.onload = (e) => setRenederedImage(e.target?.result as string);
        ...
        >
            <DialogContent>
                <DialogDescription className='flex flex-col gap-10 justify-center items-center'>
                    {renderedImage && <Image src={renderedImage} width={300} height={300} alt="selectedImage" />}
